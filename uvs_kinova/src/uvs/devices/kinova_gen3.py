...
from .rgbd_camera import RGBDVision
...
class KinovaGen3(object):
    ...
    def __init__(
        ...
        try:
            ...
            self.camera = None
            if init_camera:
                self.camera = RGBDVision(name=robot_name)


            # Gen3 services
            # -----
            # Action topic subscriber
            if not debug_camera_only:
    ...
    def _joint_state_cb(self, msg):
        ...
        """
        self.position = np.array(msg.position[:len(self.joint_names)]).astype(np.float64)
    ...
    def send_joint_angles(
        ...
        while (error_number >= 1 and angular_duration != MAX_ANGULAR_DURATION) :
            ...
            try:
                ...
                rospy.logerr("Failed to call 1ValidateWaypointList")
                return False
    ...
    def send_joint_velocities(
        ...
        if self.robot_name != "none":
            try:
                self.joint_vel_pub.publish(temp_base)
